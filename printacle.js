...
/* ---------- POSTERS ---------- */
...

    {
        ...
        description: `Premium Polyester Quality
        big size (14x16  inch)
        highly durable 
        Reusbale and easy to wash 
...
function makeProductCard(product) {
  ...
  // The Add button on the product card becomes a quantity counter
  ...
  addButton.addEventListener("click", event => {
    ...
    if (!action) {
      // First click: add one and immediately show the counter.
      addToCart(product, 1);
      renderQuickAdd();
      return;
    }
...
function openProductModal(product) {
  ...
  if (imageWrap) {
    // Rebuild only the gallery inside the existing image area.
    ...
    // One image = exactly one image, with no slider controls.
    if (images.length > 1) {
      let currentIndex = 0;
